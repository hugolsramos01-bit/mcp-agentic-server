...
description: Delegate coding tasks to user-configured Agentic MCP subagents.
---
...
```bash
agentic agents ls
agentic agents run <profile-or-provider-or-id> "<prompt>"
agentic agents show <id>
```

`ls` shows existing subagent sessions for the current workspace. Agentic MCP scopes
...
Agentic MCP agent integration.

## Choosing a profile

Choose profiles from the compact subagent profile catalog returned by
`open_workspace`. Use the profile name with `agentic agents run`. If no
...
```bash
agentic agents run <profile-or-provider> --model <model> "<prompt>"
agentic agents run <profile-or-provider> --thinking <level> "<prompt>"
```
...
the selected local agent harness; Agentic MCP does not translate values between
providers.
...
Agentic MCP tools unless the user asked for delegation, another agent's opinion,
