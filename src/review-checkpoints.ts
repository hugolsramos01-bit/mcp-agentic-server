...
const REVIEW_REF_PREFIX = "refs/agentic/review";

// Internal metadata paths that should never appear in show_changes output.
// These are Agentic MCP's own artifacts, not user changes.
// NOTE: No shell quotes — execFile passes args as array, so single quotes
// would be passed LITERALLY to git (broken on Windows).
const AGENTIC_METADATA_EXCLUSIONS = [
  ":!.agentic-checkpoints",
  ":!.agentic",
  ":!.agentic-git",
];
...
export function createReviewCheckpointManager(): ReviewCheckpointManager {
  ...
  return {
    ...
    async reviewChanges({ workspaceId, root, since = "last_shown", markReviewed = true }) {
      ...
      const patch = (await git(state.gitRoot, ["diff", "--binary", "--no-color", baseline, current, "--", ".", ...AGENTIC_METADATA_EXCLUSIONS], {
      ...
      return {
        result:
          summary.files === 0
            ? `No changes since ${since === "workspace_open" ? "workspace open" : "last shown changes"}. (Internal metadata like .agentic/ and .agentic-checkpoints/ are excluded.)`
            : `Changed ${summary.files} ${summary.files === 1 ? "file" : "files"} (+${summary.additions} -${summary.removals}). Internal metadata (.agentic/, .agentic-checkpoints/) excluded.`,
        summary,
        files,
        patch,
      };
...
async function createWorkingTreeSnapshot(gitRoot: string): Promise<string> {
  const tempDir = await mkdtemp(join(tmpdir(), "agentic-review-index-"));
  ...
  try {
    ...
    return (await git(gitRoot, ["commit-tree", tree, "-p", parent, "-m", "Agentic MCP review snapshot"], { env })).stdout.trim();
  } finally {
...
function checkpointEnv(indexPath: string): NodeJS.ProcessEnv {
  return {
    ...
    GIT_AUTHOR_NAME: "Agentic MCP",
    GIT_AUTHOR_EMAIL: "agentic@users.noreply.local",
    GIT_COMMITTER_NAME: "Agentic MCP",
    GIT_COMMITTER_EMAIL: "agentic@users.noreply.local",
  };
}
