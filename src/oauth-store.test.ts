...
const root = await mkdtemp(join(tmpdir(), "agentic-oauth-test-"));
const oauthConfig = {
...
async function testDatabaseConfiguration(stateDir: string): Promise<void> {
  ...
  try {
    ...
    const migrations = database.sqlite
      .prepare("select version, name from agentic_schema_migrations order by version")
      .all();
    assert.deepEqual(migrations, [
      { version: 1, name: "workspace-state" },
      { version: 2, name: "oauth-state" },
      { version: 3, name: "local-agent-sessions" },
      { version: 4, name: "workspace-alias" },
    ]);
  } finally {
    database.close();
  }
...
function testExpiredTokenCleanup(stateDir: string): void {
  ...
  store.saveTokenPair({
    ...
    accessToken: { clientId: client.client_id, scopes: ["agentic"], expiresAt: expiredAt },
    refreshTokenHash: "expired-refresh-hash",
    refreshToken: { clientId: client.client_id, scopes: ["agentic"], expiresAt: expiredAt },
  });
  store.close();
      ...
      store.saveTokenPair(
        {
          ...
          accessToken: { clientId: client.client_id, scopes: ["agentic"], expiresAt },
          refreshTokenHash: "new-refresh-hash",
          refreshToken: { clientId: client.client_id, scopes: ["agentic"], expiresAt },
        },
        "old-refresh-hash",
      ),
      true,
...
async function testProviderRestartRotationAndRevocation(stateDir: string): Promise<void> {
  ...
  try {
    ...
    await assert.rejects(
      secondProvider.exchangeRefreshToken(client, issued.refresh_token, ["agentic"], mcpUrl),
      InvalidGrantError,
    );
    ...
    await assert.rejects(
      secondProvider.exchangeRefreshToken(client, refreshed.refresh_token, ["agentic"], mcpUrl),
      InvalidGrantError,
    );
  } finally {
