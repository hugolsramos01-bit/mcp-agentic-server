...
for (const flag of ["-v", "--version"]) {
  const output = execFileSync("node", ["--import", "tsx", "src/cli.ts", flag], {
    encoding: "utf8",
    env: { ...process.env, AGENTIC_CONFIG_DIR: "/tmp/agentic-cli-version-test" },
  }).trim();

  assert.equal(output, packageJson.version);
}

const root = mkdtempSync(join(tmpdir(), "agentic-cli-agents-test-"));
try {
  const configDir = join(root, ".agentic");
  ...
  const output = execFileSync("node", ["--import", "tsx", "src/cli.ts", "agents", "ls"], {
    ...
    env: {
      ...
      AGENTIC_WORKSPACE_ID: "ws_current",
      AGENTIC_WORKSPACE_ROOT: projectRoot,
      AGENTIC_SUBAGENTS: "1",
