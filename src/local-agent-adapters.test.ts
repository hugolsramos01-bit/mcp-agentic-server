...
import { removeProjectNodeModulesBinFromPath } from "./local-agent-path.js";
...
{
  const agenticBin = `${process.cwd()}/node_modules/.bin`;
  const userBin = "/home/user/.local/bin";
  assert.equal(
    removeProjectNodeModulesBinFromPath([agenticBin, userBin].join(delimiter)),
    userBin,
  );

  const env = piCommandEnvironment({
    PATH: [agenticBin, userBin].join(delimiter),
  });
...

{
  ...
  const env = piCommandEnvironment({
    ...
    PATH: [agenticBin, "/home/user/.local/bin"].join(delimiter),
  });

  assert.equal(env.PATH, [agenticBin, "/home/user/.local/bin"].join(delimiter));
}
