...
try {
  ...
  await mkdir(join(root, ".agentic", "agents"), { recursive: true });
  await writeFile(
    join(root, ".agentic", "agents", "reviewer.md"),
    [
      "---",
      "name: reviewer",
  ...
  const config = loadConfig({
    AGENTIC_CONFIG_DIR: join(root, ".agentic-home"),
    AGENTIC_ALLOWED_ROOTS: root,
    AGENTIC_WORKTREE_ROOT: join(root, ".agentic", "worktrees"),
  ...
  if (platform() !== "win32") {
    ...
    const unsafeConfig = loadConfig({
      AGENTIC_CONFIG_DIR: join(root, ".agentic-unsafe-home"),
      AGENTIC_ALLOWED_ROOTS: root,
      AGENTIC_WORKTREE_ROOT: join(root, ".agentic", "unsafe-worktrees"),
      AGENTIC_AGENT_DIR: unsafeAgentDir,
      AGENTIC_OAUTH_OWNER_TOKEN: "test-owner-token-that-is-long-enough",
      PORT: "1",
    });
  ...
  await git(gitRoot, ["config", "user.email", "agentic@example.com"]);
  await git(gitRoot, ["config", "user.name", "Agentic MCP Test"]);
  ...
  if (platform() !== "win32") {
    ...
    const aliasConfig = loadConfig({
      AGENTIC_ALLOWED_ROOTS: aliasRoot,
      AGENTIC_WORKTREE_ROOT: join(aliasRoot, ".agentic", "alias-worktrees"),
      AGENTIC_AGENT_DIR: agentDir,
      AGENTIC_OAUTH_OWNER_TOKEN: "test-owner-token-that-is-long-enough",
      PORT: "1",
    });
